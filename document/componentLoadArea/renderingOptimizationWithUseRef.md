...
- 리액트는 화면을 다시 그리는 용도로 사용한다. 그런데 화면을 그릴 때 마다 상태변수에 할당할 값을 복사하기 위해 또는 새로 생성하기 위해 시스템의 리소스(CPU 또는 메모리 사용량)를 소모한다면 화면의 렌더링이 느려지는 현상이 발생하고 유저는 버벅이는 화면을 보게 될 수 있다. 컴퓨터의 속도는 충분히 빨라서 대부분의 경우 문제가 없지만, 방대한 양의 데이터가 들어 있는 값을 복사하게 되면 랜더링 속도에 악영향을 줄 수 있다.
- 따라서 컴포넌트 함수를 실행하면서 상태 변수에 할당할 대상은 화면을 그릴 때 필요한 정보를 받는다. 화면을 그릴 때 필요하지 않은 정보라면 리액트의 렌더링 사이클에서 굳이 다룰 필요가 없으며 별도의 로직으로 분리시켜 렌더링에 영향을 주지 않도록 하는 것이 좋다.
