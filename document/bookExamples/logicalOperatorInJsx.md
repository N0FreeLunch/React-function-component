...
- 자바스크립트의 모든 값이 불리언 타입으로 형변환이 된다는 것은 'OO은 OO이다.', 'OO와 OO은 OO이다.'와 같은 문장의 진술로 참/거짓이 판별되는 `A === B`, `A > B`, `value === 'hello'` 등의 프로그래밍 언어에서의 진술뿐만 아니라 단일 값 자체로도 참/거짓이 판단되기 때문에 논리학에서 볼 때는 명제라고 할 수 없지만, 프로그래밍 언어에서는 명제로 다룰 수 있는 것이다.
